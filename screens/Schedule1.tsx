...
const Schedule1: IScheduleScreen<IScheduleScreenProps> = (props) => {
    ...
    useEffect(() => {
        setState(props.route.params.datachuaduyet);
    }, [props.route]);
    const dnGet = (index: number) => {
        let url = api.API_URL + '/baotriKHChuaDuyet/' + props.route.params.idKH;
        axios.get(url).then((aData) => {
            navigation?.navigate(routes.CLIENT.DETAILSCHEDULE, { data: aData.data, index: index, chuaduyet: true, onGoBack: () => refresh() });
        });
    };
    const refresh = () => {
        let url1 = api.API_URL + '/baotriKHChuaDuyet/' + props.route.params.idKH;
    ...
    return (
        <View style={[{ flex: 1 }, styles.container]}>
            <TextView style={[styles.font_size_17]}> Tổng số lịch bảo trì chưa duyệt: {props.route.params.countchuaduyet[0].count} </TextView>
            <FlatList
                data={state}
